...
|<img src="./images/auto4option.png" width="160px"/>|**Automatisch gesetzte Nummer einer Auswahl (Zweite Nummer von zwei Optionen):** Der Solver hat in dieser Zelle automatisch die Nummer 6 gesetzt. Und zwar in der 1. Setzung einer Nummer. Nachdem die erste Option 3 nicht zum Erfolg geführt hat, hat er die zweite Option 6 gewählt. Jetzt sind alle Optionen dieser Zelle abgearbeitet.|
|<img src="./images/optionCell.png" width="160px"/>|**Kandidat-Nummern (Candidates):** Für diese Zelle wurde noch keine Nummer gesetzt. Nur noch eine der Nummern 1, 2, 6 und 9 kann gewählt werden. Das sind die Kandidaten der Zelle. Die nicht aufgeführten Nummern sind unzulässig, weil sie bereits in einer anderen Zelle des Blocks, der Reihe oder Spalte gesetzt sind.|
|<img src="./images/neccessary.png" width="160px"/>|**Notwendige Nummer (Hidden Single):** Für die nebenstehende Zelle wurde noch keine Nummer gesetzt. Kandidat-Nummern sind die drei Nummern 1, 2 und 6. Jedoch hat der Solver ermittelt, dass die Nummer 2 notwendig ist, damit das Sudoku lösbar bleibt. 2 ist eine notwendige Nummer für diese Zelle. Eine Nummer in einer Zelle ist notwendig, wenn die Nummer in ihrem Block, in ihrer Reihe oder Spalte einzig ist. D.h. sie kann nur noch hier gesetzt werden. |
|<img src="./images/indirect.png" width="160px"/>|**Indirekt unzulässige Nummer:** Für die nebenstehende Zelle wurde noch keine Nummer gesetzt. Kandidaten dieser Zelle sind die drei Nummern 3, 6 und 7. Jedoch hat der Solver ermittelt, dass die 7 indirekt unzulässig ist. Wenn man sie setzen würde, würde der Solver sofort oder einige Schritte später die Widersprüchlichkeit des Puzzles feststellen. Zur Definition der indirekten Unzulässigkeit siehe den entsprechenden Abschnitt dieser Hilfe.|
|<img src="./images/direkterSingle.png" width="160px"/> <img src="./images/indirekterSingle.png" width="160px"/>| **Single:** Eine Single-Nummer ist ein Kandidat in einer Zelle, wenn es keine weiteren Kandidaten in der Zelle gibt. In den beiden nebenstehenden Beispielen sind die 5 und die 6 Singles. Im Gegensatz zur 5 ist die 6 ein **Verstecktes Single**. Die 6 ist in dieser Zelle ein verstecktes Single, weil die anderen Kandidaten, die rote 1 und 3, indirekt unzulässig sind.|
|<img src="./images/nochoice.png" width="160px"/>|**Widerspruch - Leere Option:** Für diese Zelle wurde noch keine Nummer gesetzt. Allerdings gibt es keine Kandidaten-Nummer mehr, die noch gesetzt werden könnte. D.h. das Sudoku ist widersprüchlich. Wenn das Sudoku noch erfolgreich gelöst werden soll, müssen ein oder mehrere der bisherigen Nummernsetzungen zurückgenommen werden. Tritt während der automatischen Ausführung eine solche Zelle auf, schaltet der Solver in den Rückwärts-Modus um.|
|<img src="./images/twoNeccessary.png" width="160px"/>|**Widerspruch - zwei notwendige Nummern:** Für diese Zelle wurde noch keine Nummer gesetzt. Kandidaten-Nummern sind 1, 2, 3, 4, 5 und 6. Jedoch hat der Solver zwei verschiedene notwendige Nummern für diese Zelle ermittelt: 1 und 2. Das geht natürlich nicht. Es können in einer Zelle nicht zwei Nummern gleichzeitig gesetzt werden. D.h. das Sudoku ist widersprüchlich. Wenn das Sudoku noch erfolgreich gelöst werden soll, müssen ein oder mehrere der bisherigen Nummernsetzungen zurückgenommen werden. Tritt während der automatischen Ausführung eine solche Zelle auf, schaltet der Solver in den Rückwärts-Modus um.|
|<img src="./images/widernotwendig.png" width="160px"/>|**Widerspruch - notwendig-indirekt unzulässig:** Für diese Zelle wurde ermittelt, dass die 8 gleichzeitig notwendig und indirekt unzulässig ist. Ein Widerspruch.|
...
Für eine noch nicht belegte Zelle der Sudoku-Tabelle unterscheiden wir Kandidaten-Nummern und unzulässge Nummern. Für eine unbelegte Zelle ist eine Nummer **unzulässig**, wenn in dem Block, in der Reihe oder Spalte dieser Zelle eine andere Zelle existiert, in der diese Nummer gesetzt ist. Alle anderen Nummern heißen **Kandidaten-Nummern (Candidates)** dieser Zelle. In einer unbelegten Zelle werden die Kandidaten-Nummern der Zelle angezeigt.

### Notwendige Nummern

![Lazy notwendig](./images/lazynotwendig.png)
Eine Kandidaten-Nummer in einer Zelle ist notwendig, wenn die Nummer in ihrem Block, in ihrer Reihe oder Spalte einzig ist. D.h. sie kann nur hier gesetzt werden. Im Bild ist die grüne 4 notwendig, weil sie in ihrer Zeile kein weiteres mal zulässig ist. Im Lazy-Auswertungsmodus zeigt der Solver den die Notwendigkeit verursachenden Block, Spalte oder Reihe an, wenn man die Zelle mit der notwendigen Nummer selektiert hat. Die roten Pfeile zeigen für jede offene Zelle der Zeile, warum in ihnen die 4 unzulässig ist. 

Hinweis: Im Wikipedia.Artikel [Wikipedia](https://en.wikipedia.org/wiki/Glossary_of_Sudoku) werden notwendige Nummern werden als "Hidden Singles" bezeichnet. Wir wollen diese Bezeichnung hier nicht übernehmen, weil wir diese Bezeichnung schon anderweitig benutzen. Wir bezeichnen eine Kandidaten-Nummer einer Zelle als Verstecktes Single, wenn alle übrigen Kandidaten der Zelle indirekt unzulässig sind. Soweit richtig verstanden, gibt es für diese Konstellation im Wikipedia-Artikel keinen eigenen Begriff.

### Indirekt unzulässige Nummern (Eliminated Candidates)

**Unzulässige Nummern** sind Nummern, die in einem Block, in einer Spalte oder Zeile bereits einmal existieren. In einer ungesetzten Zelle werden die  unzulässigen Nummern nicht angezeigt. Anders dagegen indirekt unzulässige Kandidaten. Indirekt unzulässige Kandidaten werden rot angezeigt. Eine Kandidaten-Nummer ist **indirekt unzulässig**, wenn ihre Setzung in der Zelle das Puzzle widerspruchsvoll macht.

Wie kann man rein logisch, ohne Back-Tracking, erkennen, dass eine Kandidaten-Nummer indirekt unzulässig ist (rot ist)? Der vorliegende Solver kennt bislang folgende unterschiedliche logische Kriterien. In der Literatur werden zahlreiche weitere Kriterien genannt, die jedoch nicht notwendig die Erkennung indirekt unzulässiger Kandidaten verbessern. Sie sind manchmal nur Alternativen. Grundsätzlich ist die rein logische Erkennung der indirekten Unzulässigkeit unvollständig. D.h. es gibt so schwere Puzzles, dass deren Lösung nur unter Verwendung von Back-Tracking erreicht werden kann. Von diesem Solver unterstützte Kriterien für indirekte Unzulässigkeit von Kandidaten.

1. **Kriterium: Notwendige Nummer.** Eine Kandidaten-Nummer ist indirekt unzulässig wegen einer notwendigen Nummer, wenn sie in ihrer Spalte, Reihe oder ihrem Block auch als notwendige Nummer auftritt. Im nachfolgenden Beispiel sind die roten Nummern 8 wegen der grünen 8 indirekt unzulässig. Die grüne 8 ist notwendig, weil sie in ihrem Block einzig ist, also in dem Block kein weiteres mal zulässig ist.![Indirekt wegen notwendig](./images/indirektwgnotwendig.png)

1. **Kriterium: Single:** Eine Kandidaten-Nummer ist indirekt unzulässig wegen einer Single-Nummer, wenn sie in der Spalte, Reihe oder dem Block der Single-Nummer ein zweites mal auftreten würde.

1. **Kriterium: Nacktes Paar (Naked Pair).** Eine Kandidaten-Nummer ist indirekt unzulässig, wenn es in einem Block, einer Zeile oder Spalte Paare gibt und Nummern dieser Paare zusätzlich in weiteren Zellen dieses Blocks, dieser Spalte oder Zeile auftauchen. Im nachfolgenden Beispiel ist das 4-6-Paar ein nacktes Paar. Das 4-6-Paar macht in seinem Block alle 4 und 6 indirekt unzulässig. Der Grund: Das Paar bedeutet, dass die 4 und die 6 auf jeden Fall in einer der beiden Zellen des Paares gesetzt werden muss. Aktuell steht nur noch nicht fest, ob die 6 oder die 4 oben ist. Fest steht aber jetzt schon, dass in den übrigen Zellen des Blocks keine 4 oder 6 mehr vorkommen können. Die 4 und 6 sind hier indirekt unzulässig. Diese Blockdarstellung mit den gestrichelten Kanten zeigt der Solver nur im Lazy-Auswertungsmodus, wenn man eine Zelle mit indirekt unzulässigen Nummern selektiert hat.![Indirekt unzulässig](./images/indirektWegenPairing.png)

1. **Kriterium: Verstecktes Paar (Hidden Pair).** In einem Block, einer Spalte oder Zeile kann es ein verstecktes Paar geben. Ein verstecktes Paar besteht aus zwei Zellen, die zwei gemeinsame Nummern haben, im Beispiel 3 und 5, die in den übrigen Zellen nicht vorkommen. Daneben können sie weitere Nummern haben. Am Ende können in diesen beiden Zellen nur die beiden Nummern 3 und 5 untergebracht werden. Deshalb müssen die übrigen Nummern der beiden Zellen eliminiert werden.
![Verstecktes Paar](./images/hiddenpair.png)

1. **Kriterium Überschneidung (Pointing Pair)** Ein Block und eine Spalte oder Zeile überschneiden sich. In der Spalte gibt es Nummern, die nur in den gemeinsamen Zellen mit dem Block auftauchen. Im Beispiel die 7. Damit es am Ende in der Spalte überhaupt eine 7 gibt, muss eine 7 in der Spalte gewählt werden. Dies wiederum bedeutet, dass die Nummern 7 in dem Block neben der Spalte gestrichen werden müssen.
![Überschneidung](./images/ueberschneidung.png)

### Auswertungsmethoden

Die beiden Beispieltabellen im vorigen Abschnitt zeigen den Solver in unterschiedlichen Auswertungsmodi. Man sieht sofort, dass die Paarerkennung im Strikt-Modus wegen der Fülle indirekt unzulässiger Nummern nur noch schwer möglich ist. Abhilfe schafft hier die Lazy-Auswertung.

1. **Lazy Auswertung:** Diese Auswertungsmethode praktiziert eine verzögerte Auswertung. Die Auswertung erfolgt nur soweit, bis die nächste notwendige Nummer oder die nächste Single-Nummer oder die nächste versteckte Single-Nummer bestimmt ist. Diese Nummer wird dann gesetzt.

1. **Strikte Auswertung:** Diese  Auswertungsmethode führt eine vollständige Auswertung durch: Alle aktuell indirekt unzulässigen Nummern, alle notwendigen Nummern, alle Singles und versteckten Singles werden ermittelt. Danach können diese Nummern Schritt für Schritt gesetzt werden.
...
1. **Strikt -** : Die errechneten indirekten unzulässigen Nummern werden ausgeblendet. Das nachfolgende Bild zeigt die vor vorige Tabelle mit dem Puzzle-Namen 'Schwer 2 Singles' im Strikt-Minus-Modus. ![Strikt Minus](./images/striktminus.png)
